...
class PawGPS(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        if "ip" not in self.options or ("ip" in self.options
                                        and self.options["ip"] is None):
            logging.info(
                "PAW-GPS: No IP Address in the config file is defined, it uses the default (192.168.44.1:8080)"
            )
