    ...
    def on_internet_available(self, agent):
        with self.lock:
            if self.ready:
                ...
                if new_np_files:
                    logging.debug(
                        "NET-POS: Found %d new net-pos files. Fetching positions ...", len(new_np_files))
                    display.set(
                        'status', f"Found {len(new_np_files)} new net-pos files. Fetching positions ...")
                    ...
                    for idx, np_file in enumerate(new_np_files):
                        ...
                        try:
                            ...
                            logging.error(
                                "NET-POS: %s - JSONDecodeError: %s, removing it...", np_file, js_e)
                            os.remove(np_file)
                            continue
                        ...
                        display.set(
                            'status', f"Fetching positions ({idx + 1}/{len(new_np_files)})")
                        display.update(force=True)
