...
class Handler:
    def __init__(self, config, agent, app):
        ...
        self._app.add_url_rule("/shutdown",
                               "shutdown",
                               self.with_auth(self.shutdown),
                               methods=["POST"])
        self._app.add_url_rule("/reboot",
                               "reboot",
                               self.with_auth(self.reboot),
                               methods=["POST"])
        self._app.add_url_rule("/restart",
                               "restart",
                               self.with_auth(self.restart),
                               methods=["POST"])

        # inbox
        self._app.add_url_rule("/inbox", "inbox", self.with_auth(self.inbox))
        self._app.add_url_rule("/inbox/profile", "inbox_profile",
                               self.with_auth(self.inbox_profile))
        ...
        self._app.add_url_rule("/inbox/<id>/<mark>", "mark_message",
                               self.with_auth(self.mark_message))
    ...
    def _check_creds(self, u, p):
        ...
        return secrets.compare_digest(
            u, self._config["username"]) and secrets.compare_digest(
    ...
    def plugins(self, name, subpath):
        ...
        if name == "toggle" and request.method == "POST":
            ...
            return ("success" if plugins.toggle_plugin(request.form["plugin"],
                                                       checked) else "failed")

        if (name in plugins.loaded and plugins.loaded[name] is not None
